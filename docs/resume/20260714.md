...
pgt0409@gmail.com | gyeongtae@apache.org

Data Engineer로 커리어를 시작해 대용량 배치·파이프라인 설계와 온프레미스·클라우드 인프라 구축 역량을 쌓았고, 현재는 TMAP에서 Data Analytics Engineer로서 데이터를 비즈니스 인사이트와 의사결정으로 연결하는 역할을 맡고 있는 5년차 데이터 전문가입니다. MAU 1,500만 규모의 TMAP 주행 데이터 파이프라인 설계 및 최적화, 메달리온 아키텍처 기반 데이터 플랫폼 구축까지 데이터 인프라의 전 주기(Full Lifecycle)를 경험했습니다. Apache Zeppelin Committer(ASF, 2025.08~)로서 글로벌 오픈소스에도 지속 기여하고 있습니다.

## 핵심 성과 (Key Achievements)

- Spark 파티셔닝 재설계 + Upsert 전략으로 일 수십억 건 배치 처리 시간 80% 단축 (5시간 → 1시간)
- Data Quality Check 자동화 및 Data Lineage 조회용 MCP 개발 경험으로 영향도 분석 시간 단축
- 메달리온 아키텍처 도입으로 데이터 처리 표준화, Tableau Prep 수동 전처리 제거
- 배치 파이프라인 Monorepo 통합 + 표준 템플릿 적용 → 신규 개발 시간 50% 단축

## 경력 요약 (Data Engineer → Data Analytics Engineer)

- Data Analytics Engineer | 티맵모빌리티 (2025.02 ~ 현재)
- Data Engineer | 펫프렌즈 (2023.10 ~ 2025.02)
- Data Engineer | 카테노이드 (2021.06 ~ 2023.10)
...
**Apache Zeppelin Committer (ASF, 2025.08 ~ 현재)**

- 병합 PR 25건+, 컨트리뷰터 상위 30명 이내
- Bug Fix · Test Infra · Refactoring · Docs 등 Full Contribution Cycle 참여
- 사내 운영 경험 기반의 실사용 관점 개선 주도

## 기술 블로그

누적 방문 60만+회 | pinggoopark.tistory.com

주요 글:
- 메달리온 아키텍처로 Tableau Prep 대체하기 (Medium)
- AWS MWAA의 백엔드로 AWS Secrets Manager 도입기 (Medium)
...
2025.02 - 재직중 (1년 5개월) | 정규직 | 데이터 분석 엔지니어 매니저

**회사 소개**: 국내 1위 내비게이션 서비스 (MAU 1,500만, 누적 가입자 2,500만) — https://www.tmapmobility.com
...
- 분석가와 협업하여 주행 패턴·경로 데이터 기반의 인사이트를 발굴, 비즈니스 의사결정에 활용
...
**데이터 엔지니어링 & 개발 문화 개선**
- 분석팀 내 산재된 배치 파이프라인의 코드 구조를 표준 템플릿으로 통합, 신규 파이프라인 개발 시간 약 50% 단축
- Bitbucket → GitHub 마이그레이션을 주도하고 CI/CD 기반 Jar 빌드 자동화 및 코드 리뷰 프로세스를 도입하여 배포 안정성 확보 및 협업 효율 향상

### (주)펫프렌즈

2023.10 - 2025.02 (1년 5개월) | 정규직 | 데이터 엔지니어 매니저

**회사 소개**: 반려동물 커머스 1위 (2023년 기준 거래액 1,234억, 매출 1,030억) — https://pet-friends.co.kr

**데이터 아키텍처 설계 & 거버넌스**
- 데이터 처리 일관성 확보를 위해 메달리온 아키텍처(Medallion Architecture)를 논리·물리 구조 모두에 적용, Bronze/Silver/Gold 레이어 기반의 데이터 품질 단계별 관리 체계를 수립
...
- Amplitude, Airbridge, Braze 등 3rd Party 데이터 소스를 통합 수집하는 파이프라인을 구축, 마케팅 성과 분석의 데이터 기반을 확보
...
**기술 블로그**
- 메달리온 아키텍처로 Tableau Prep 대체하기 (https://medium.com/p/cfbb)
- AWS MWAA의 백엔드로 AWS Secrets Manager 도입기 (https://medium.com/p/addfe)

### 카테노이드 주식회사

2021.06 - 2023.10 (2년 5개월) | 정규직 | 데이터 엔지니어 대리

**회사 소개**: 국내 최초 온라인 비디오 플랫폼 (Kollus/Kollus Live, E-러닝 분야 기업 고객 점유율 1위) — https://www.catenoid.net
...
2012.03 - 2018.02 졸업 | 전기공학과

PV+ESS의 개인 가정의 전력사용 예측을 통한 효율적 사용 방안에 대한 연구

## 수상/자격증/기타

### Committer of Apache Zeppelin

2025.08 ~ 현재

Apache Software Foundation(ASF)의 공식 Committer로 선정되어 Apache Zeppelin 프로젝트에 지속적으로 기여하고 있습니다.

**기여 규모**
- 병합 PR 25건 이상, 전체 컨트리뷰터 상위 30명 이내
- 이슈 생성 → PR 작성 → 코드 리뷰 → 병합까지 Full Contribution Cycle 전 과정에 참여

**주요 기여 영역**
- Bug Fix: 노트북 파일 경로의 URL 인코딩 이슈 수정 등 실사용 환경에서 발견한 버그를 직접 리포트하고 해결
- Test Infra: Selenium 테스트 환경을 Chrome → Firefox로 전환하고 의존 라이브러리(Spark 등)를 업그레이드하여 CI 안정성 개선
- Refactoring & Docs: 코드 리팩터링, 테스트 커버리지 확대, 문서 개선 등 프로젝트 유지보수성 향상에 기여

**기여 배경**
- 사내 Zeppelin 도입·운영 경험에서 발견한 실사용 이슈를 업스트림에 직접 기여하는 방식으로 오픈소스 활동을 시작
...
2017.11 ~ 2020.11 (3년) | 기술부 | 광학 하드웨어 엔지니어

- 디스플레이용 분광측정기·막두께 측정 시스템의 설치·운영·유지보수를 담당
