"""
uv run python stock_analysis/20251010_종목별시가총액/KOSPI_오늘기준_시가총액_상위30개.py
...
def get_top_kospi_by_mcap(n=100, years_ago=0):
    ...
    # 상위 30개 종목들의 시가총액 합계 계산
    top30_mcap = df["시가총액"].head(n).sum()
    df["시가총액비율(%)"] = (df["시가총액"] / top30_mcap * 100).round(3)
...
if __name__ == "__main__":
    # ✅ 오늘 기준 데이터
    date, top100 = get_top_kospi_by_mcap(n=30, years_ago=0)
    ...
    output_path = f"stock_analysis/20251010_종목별시가총액/KOSPI_오늘기준_시가총액_상위30개_{date}.csv"
