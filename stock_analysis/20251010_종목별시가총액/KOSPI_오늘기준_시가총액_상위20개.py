"""
uv run python stock_analysis/20251010_종목별시가총액/KOSPI_오늘기준_시가총액_상위20개.py
uv run streamlit run stock_analysis/20251010_종목별시가총액/KOSPI_오늘기준_시가총액_상위20개_streamlit.py
"""
...
def get_top_kospi_by_mcap(n=100, years_ago=0):
    """코스피 시가총액 상위 n개를 가져옵니다. (기준: 오늘)"""
    today = datetime.now()
    ...
    df["시가총액(조)"] = (df["시가총액"] / 1e12).round(3)

    # 상위 20개 종목들의 시가총액 합계 계산
    top20_mcap = df["시가총액"].head(n).sum()
    df["시가총액비율(%)"] = (df["시가총액"] / top20_mcap * 100).round(3)
...
if __name__ == "__main__":
    # ✅ 오늘 기준 데이터
    date, top100 = get_top_kospi_by_mcap(n=20, years_ago=0)
    ...
    output_path = f"stock_analysis/20251010_종목별시가총액/KOSPI_오늘기준_시가총액_상위20개_{date}.csv"
